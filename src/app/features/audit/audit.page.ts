...
<tr *ngFor="let x of visible"><td>{{x.createdAtUtc|date:'short'}}</td><td><b>{{x.action}}</b></td><td>{{x.entityType}}</td><td><code>{{x.entityId}}</code></td><td>{{x.userId||'System / AI'}}</td></tr>
</tbody></table></section>`})
export class AuditPage implements OnInit{
  ...
  exportCsv(){
    ...
    const lines=[['Time','Action','Entity','Entity ID','User'].map(esc).join(','),...this.visible.map(x=>[x.createdAtUtc,x.action,x.entityType,x.entityId,x.userId||'System / AI'].map(esc).join(','))];
