...
<qai-page-header title="Prospect Discovery" subtitle="Define who you want to sell to, collect market evidence and prioritize companies showing real buying intent.">
  <button (click)="load()">↻ Refresh</button><button (click)="icpOpen=true">+ Ideal customer profile</button><button class="primary" (click)="prospectOpen=true">+ Import prospect</button>
</qai-page-header>
<div class="metrics">
...
<div class="grid2">
  <section class="panel"><header><div><b>Ideal customer profiles</b><span>Discovery and scoring rules</span></div></header>
    <div class="gap" *ngFor="let x of icps"><div><b>{{x.name}}</b><span>{{x.industry||'All industries'}} · {{x.countriesCsv||'All countries'}} · {{x.minimumEmployees||0}}–{{x.maximumEmployees||'∞'}} employees</span></div><em>{{x.active?'Active':'Paused'}}</em></div>
...
<table><thead><tr><th></th><th>Company</th><th>Decision maker</th><th>Market</th><th>Fit</th><th>Intent</th><th>Priority</th><th>Status</th><th></th></tr></thead>
...
export class DiscoverPage implements OnInit{
  overview:any={};icps:any[]=[];prospects:any[]=[];minimumScore=0;selectedIds=new Set<string>();listName='';icpOpen=false;prospectOpen=false;signalOpen=false;signalFor:any;
  ...
  load(){this.data.overview().subscribe(r=>this.overview=r);this.data.icps().subscribe(r=>this.icps=r);this.loadProspects()}
  loadProspects(){this.data.prospects(this.minimumScore).subscribe(r=>this.prospects=r)}
  ...
  saveIcp(){this.data.createIcp(this.icp).subscribe(r=>{this.icps.push(r);this.icpOpen=false})}
  ...
  createList(){this.data.createTargetList({name:this.listName,description:'Selected from prospect discovery',icpProfileId:null,dynamic:false}).subscribe(list=>this.data.addMembers(list.id,[...this.selectedIds]).subscribe(()=>{alert('Target list created and ready for a campaign.');this.selectedIds.clear();this.listName=''}))}
}
